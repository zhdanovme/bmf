...
### 11.1. Run Epic/TC Namespace Validation

Execute namespace validation:
```bash
npx ts-node utils/validate-tcs-epics.ts {folder}
```

This checks:
- Every `tc:DOMAIN:*` has a corresponding `epic:DOMAIN` in `_epics.yaml`
- Every `epic:DOMAIN` has at least one `tc:DOMAIN:*`

If validation fails, fix by either:
- Adding missing epics to `_epics.yaml`
- Renaming test cases to match existing epic domains
...
- Always run validate-tcs-epics.ts at the end to check namespace consistency
- **TC domain must match an epic domain**: `tc:DOMAIN:*` requires `epic:DOMAIN` in `_epics.yaml`
